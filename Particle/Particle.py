...
class Particle:
    ...
    def Update(self, noise_map):
        if (self._x_pos > 0 and self._y_pos > 0 and self._x_pos < settings.WINDOW_WIDTH and self._y_pos < settings.WINDOW_HEIGHT):
            self._x_accel = settings.MAGNITUDE * math.cos(noise_map[int(self._x_pos / 10)][int(self._y_pos / 10)] * math.pi * settings.NOISE_MULTIPLIER)
            self._y_accel = settings.MAGNITUDE * math.sin(noise_map[int(self._x_pos / 10)][int(self._y_pos / 10)] * math.pi * settings.NOISE_MULTIPLIER)
